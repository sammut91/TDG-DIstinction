...
FastMinion::FastMinion(int xPos, int yPos)
{
	m_Position->x = xPos;
	m_Position->y = yPos;
	m_Velocity = new Vector2D(0.0, 0.0);
...
FastMinion::FastMinion(int xPos, int yPos, SDL_Renderer* renderer, Path* p)
{
...
FastMinion::FastMinion(int xPos, int yPos, SDL_Renderer* renderer, Path* p, int damage, int health)
{
	m_Position = new Point2D(xPos, yPos);
	m_Velocity = new Vector2D(0.0, 0.0);
	m_Accel = new Vector2D(0.0, 0.0);
...
bool FastMinion::Initialise(SDL_Renderer* renderer)
{
	bool success = true;
	this->setVelocity(0.0, 0.0);
	if (!m_Texture->loadFromFile("GreenBlob.png", renderer))
	{
...
void FastMinion::Update(float timeStep)
{
...
void FastMinion::Render(SDL_Renderer* r)
{
	if (m_Texture != NULL)
	{
...
Vector2D FastMinion::Seek(Point2D* targetPos)
{
...
Vector2D FastMinion::FollowPath()
{
	Vector2D force;
...
Vector2D FastMinion::CalculateForce(float timeStep)
{
	Vector2D force;
...
void FastMinion::AddPath(Path* p)
{
	if (p != NULL)
		this->m_Path = p;
}

bool FastMinion::AtDestination()
{
	bool atDest = false;
