...
int main(int argc, char* args[])
{
	...
	{
		//Create window
		window = SDL_CreateWindow("Blob Tower Defense", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
		if (window == NULL)
		{
			...
			else
			{
				...
				while (game.Running())
				{
					...
					if (!game.isPaused()) 
					{
						...
						if (loops == 10)
						{
							game.m_ScoreText.str("");
							game.m_CurrencyText.str("");
							game.m_Score += 1;
							game.m_ScoreText << (int)game.m_Score;
